...
```bash
lambda-container-fuse-workers$ sam build
```
...
```bash
lambda-container-fuse-workers$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
lambda-container-fuse-workers$ sam local start-api
lambda-container-fuse-workers$ curl http://localhost:3000/
```
...
```bash
lambda-container-fuse-workers$ sam logs -n HelloWorldFunction --stack-name lambda-container-fuse-workers --tail
```
...
```bash
lambda-container-fuse-workers$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name lambda-container-fuse-workers
```
